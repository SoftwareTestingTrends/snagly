...
description: Synthesize outputs from one or more of the other testing skills in this toolkit (scenario-mapper CSVs, flow-runner run reports, bug-triage writeups, accessibility-audit findings, network-assertion checks, cross-browser matrices) — potentially from several sessions across a testing cycle — into a single prioritized, human-readable report. Use whenever the user wants a summary, status update, or write-up of testing results — "summarize what we found," "give me a report I can send," "what's our testing status," "write up this testing cycle," "put this in a doc for the team" — especially when more than one prior testing run or skill's output needs to be pulled together. Also covers register-shaped questions about accumulated results rather than a document request — "which defects are still open," "what have we found across these cycles," "summarize the test runs," or any question that spans several run directories. This skill does not drive a browser or run any tests itself — it only reads and synthesizes artifacts the other skills already produced. If nothing has been tested yet, the answer is to run one of the other skills first, not to fabricate a report from nothing.
---
